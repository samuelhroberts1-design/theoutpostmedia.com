...
function initRandomButton() {
  ...
  btn.addEventListener('click', () => {
    const pool = articles.filter(a => {
      // Exclude combined/full essays to avoid duplicating content
      const slug = a.slug;
      return !slug.endsWith('-full') && !slug.endsWith('-combined');
    });
...
function initSearch() {
  ...
  // Deduplicated search pool — exclude -full and -combined slugs
  const pool = articles.filter(a => {
    const s = a.slug;
    return !s.endsWith('-full') && !s.endsWith('-combined');
  });
...
function initAllArticlesPage() {
  ...
  // Deduplicated pool sorted by date — exclude -full/-combined
  const pool = articles
    .filter(a => !a.slug.endsWith('-full') && !a.slug.endsWith('-combined'))
    .sort(byDateDesc);
